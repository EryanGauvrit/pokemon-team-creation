<template>
  <main class="container max-w-4xl m-auto mt-10">
      ...
      <div class="flex justify-center gap-2 mt-4">
          ...
          <button @click="getPokemonsByName" class="bg-primary text-background py-2 px-4 rounded-sm ml-2">
      ...
      <div v-if="typesList.length > 0" class="flex flex-wrap justify-center gap-y-2 gap-x-4 mt-4">
          <button @click="getPokemons(pokeType.name)" v-for="pokeType in typesList" class="bg-primary text-background py-2 px-4 rounded-sm">{{ pokeType.name }}</button>
      </div>
