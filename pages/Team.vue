<template>
    <main class="container max-w-4xl m-auto mt-10">
        <div class="flex justify-between items-center">
            ...
            <p class="text-xl font-bold">{{ team.length }}/6</p>
        </div>
        <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 py-10">
            <div v-for="pokemon in team" :key="pokemon.id" class="bg-card text-card-foreground border rounded-md shadow-sm hover:scale-[1.05] duration-200 overflow-hidden">
                ...
                <div class="flex justify-center gap-4 p-5">
                    <NuxtLink :to="`/pokemon/${pokemon.pokedexId}`" class="bg-primary text-background py-2 px-4 rounded-md hover:bg-secondary duration-200">
                        <Icon name="ic:baseline-remove-red-eye" class="w-6 h-6" />
                    </NuxtLink>
                    <button @click="removeFromTeam(pokemon)" class="bg-destructive text-background py-2 px-4 rounded-md hover:bg-secondary duration-200">
                        <Icon name="ic:baseline-delete-forever" class="w-6 h-6" />
                    </button>
                </div> 
            </div>  
        </section>
